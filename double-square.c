...
main(int argc, char **argv)
{
	...
	window.window_size.width  = 300;
	window.window_size.height = 300;

	setting.color_in = 0xffffffff;
	setting.color_out = 0xffffffff;

	for (i = 1; i < argc; i++) {
		if ((strcmp("-t", argv[i]) == 0) && (i + 1 < argc) && !setting.title) {
			setting.title = strdup(argv[i + 1]);
			i++;
		} else if ((strcmp("-ci", argv[i]) == 0) && (i + 1 < argc)) {
			sscanf(argv[i + 1], "%x", &setting.color_in);
			i++;
		} else if ((strcmp("-co", argv[i]) == 0) && (i + 1 < argc)) {
			sscanf(argv[i + 1], "%x", &setting.color_out);
			i++;
		} else if (strcmp("-h", argv[i]) == 0) {
			usage(EXIT_SUCCESS);
		} else if ((strcmp("-f", argv[i]) == 0) && (i + 1 < argc) && !setting.fifo_path) {
			setting.fifo_path = strdup(argv[i + 1]);
			i++;
		} else {
			usage(EXIT_FAILURE);
		}
	...
	if (setting.fifo_path)
		if (pthread_create(&thread, NULL, &fifo_thread, &setting) < 0) {
...
usage(int error_code)
{
	fprintf(stderr, "Usage: square [OPTIONS]\n\n"
		...
		"  -ci <inner color>\tSpecify color of inner rect(default = 0xffffffff)\n"
		"  -co <outer color>\tSpecify color of outer rect(default = 0xffffffff)\n"
		"  -f <path to fifo>\tSpecify path to fifo(default = Not use fifo)\n"
		"  -h\tThis help text\n\n");
...
redraw(void *data, struct wl_callback *callback, uint32_t time)
{
	...
	draw_rectangle(window,       -1,  1 - 0.5,   2, 0.5, display->setting->color_out);
	draw_rectangle(window,       -1,       -1,   2, 0.5, display->setting->color_out);
	draw_rectangle(window,       -1, -1 + 0.5, 0.5,   1, display->setting->color_out);
	draw_rectangle(window,  1 - 0.5, -1 + 0.5, 0.5,   1, display->setting->color_out);
	draw_rectangle(window, -1 + 0.5, -1 + 0.5,   1,   1, display->setting->color_in);
...
static void *fifo_thread(void *arg)
{
	...
	while (1) {
		...
		fp = fopen(setting->fifo_path, "r");
		if (!fp) {
			fprintf(stderr, "Error: open fifo(%s)\n", setting->fifo_path);
			break;
		}

		fgets(buff, sizeof(buff), fp);
		if (strncmp(buff, "alpha inner ", 12) == 0) {
			val = atoi(&buff[12]) & 0xff;
			setting->color_in &= 0x00ffffff;
			setting->color_in |= (val << 24);
		} else if (strncmp(buff, "alpha outer ", 12) == 0) {
			val = atoi(&buff[12]);
			setting->color_out &= 0x00ffffff;
			setting->color_out |= (val << 24);
		}
